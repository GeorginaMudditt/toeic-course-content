/**
 * Generate Finance and the Economy worksheet pronunciation audio via ElevenLabs TTS.
 * Saves to public/vocab-audio/finance-and-the-economy-vocab-audio/{word}.mp3
 *
 * Usage:
 *   npx tsx scripts/generate-finance-and-the-economy-vocab-tts.ts
 *   npx tsx scripts/generate-finance-and-the-economy-vocab-tts.ts --dry-run
 *   npx tsx scripts/generate-finance-and-the-economy-vocab-tts.ts --skip-existing
 */
...
const WORDS: ReadonlyArray<{ word: string; tts_text?: string; trim_prefix?: string }> = [
  ...
  { word: 'investor' },
  { word: 'currency' },
  { word: 'commodities', tts_text: 'commodities.' },
  { word: 'stock market' },
  { word: 'trade', tts_text: 'trade.' },
  { word: 'economic' },
  { word: 'deficit' },
  { word: 'recession' },
]
...
function trimAudioFromBuffer(input: Buffer, startSeconds: number): Buffer {
  const tmpIn = join(tmpdir(), `finance-tts-trim-in-${Date.now()}.mp3`)
  const tmpOut = join(tmpdir(), `finance-tts-trim-out-${Date.now()}.mp3`)
...
async function applyTrimPrefix(audioBuffer: Buffer): Promise<Buffer> {
  const tmpFull = join(tmpdir(), `finance-tts-full-${Date.now()}.mp3`)
