/**
 * Generate Company Finance worksheet pronunciation audio via ElevenLabs TTS.
 * Saves to public/vocab-audio/company-finance-vocab-audio/{word}.mp3
 *
 * Usage:
 *   npx tsx scripts/generate-company-finance-vocab-tts.ts
 *   npx tsx scripts/generate-company-finance-vocab-tts.ts --dry-run
 *   npx tsx scripts/generate-company-finance-vocab-tts.ts --skip-existing
 */
...
const OUTPUT_DIR = join(process.cwd(), 'public', 'vocab-audio', 'company-finance-vocab-audio')
const CACHE_DIR = join(process.cwd(), '.cache', 'company-finance-vocab-audio')

const WORDS: ReadonlyArray<{ word: string; tts_text?: string; trim_prefix?: string }> = [
  { word: 'profit and loss' },
  { word: 'shareholder' },
  { word: 'financial year' },
  { word: 'dividends', tts_text: 'dividends.' },
  { word: 'accounting' },
  { word: 'expenses', tts_text: 'expenses.' },
  { word: 'turnover' },
  { word: 'depreciation' },
  { word: 'balance sheet' },
  { word: 'assets', tts_text: 'assets.' },
  { word: 'market value' },
  { word: 'liabilities', tts_text: 'liabilities.' },
  { word: 'equity' },
  { word: 'cashflow' },
  { word: 'credit period' },
  { word: 'credit terms' },
...
function trimAudioFromBuffer(input: Buffer, startSeconds: number): Buffer {
  const tmpIn = join(tmpdir(), `company-finance-tts-trim-in-${Date.now()}.mp3`)
  const tmpOut = join(tmpdir(), `company-finance-tts-trim-out-${Date.now()}.mp3`)
...
async function applyTrimPrefix(audioBuffer: Buffer): Promise<Buffer> {
  const tmpFull = join(tmpdir(), `company-finance-tts-full-${Date.now()}.mp3`)
